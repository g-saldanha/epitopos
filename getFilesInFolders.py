...
dirr = "D:\\Camila Leite\\Datasets\\Biologia (Gabriel)\\New_MASTERMoveletsPivots\\epitopos_ED\\Porcentage_10_Witout_Last_Prunning"
os.chdir("D:\\Camila Leite\\Datasets\\Biologia (Gabriel)\\New_MASTERMoveletsPivots\\epitopos_ED\\Porcentage_10_Witout_Last_Prunning")
...
# dataset_train.to_csv(dirr+"\\train.csv", index = None, header=True)
...
for directory in directories: 
    ...
    if(os.path.isdir(new_folder)):
        print(new_folder)
        dataset_aux_train = pd.read_csv(directory + "\\test.csv")
        dataset_aux_train = dataset_aux_train[dataset_aux_train.columns[:-1]]
        dataset_test = dataset_test.join(dataset_aux_train)

dataset_test = dataset_test.join(labels)
dataset_test.to_csv(dirr+"\\test.csv", index = None, header=True)